...
- 状態: 実資金パイロット用
- 作成日: 2026年8月31日
- 基準ルール: [実運用手順書 v0.1](live-operation-playbook-v0.1.md)、[判定・運用ルール v0.3](tenbagger-rule-v0.3.md)
...
日次タスクが行うのは情報収集、ルール判定、注文案の作成までである。証券会社への注文送信は行わない。注文がある日だけ、利用者が翌営業日8:45〜8:55に約10分の注文前チェックを行い、承認した注文を手入力する。18:30以後に重要開示、売買停止、取引所・証券会社障害が発生し得るため、この確認は省略しない。
...
必ず operations/private/state.json と前回の handoff.json を正本として再開し、前回の開示カットオフ後から今回のJSTカットオフまでを確認してください。保有銘柄、未処理注文、監視銘柄を対象に日次イベントを確認し、期限が来た週次・月次・四半期・本決算レビューだけを同じ実行へ含めてください。

scripts/daily_operation.py prepare で当日フォルダを作成または再開し、report.md、sources.csv、orders.csv、必要な個別判断ログ、handoff.json を更新してください。注文候補は常に PROPOSED とし、証券会社へ注文を送信しないでください。重要な対象を確認できなければ failed としてカットオフを進めず、取得できなかった内容を残してください。完了時は complete を実行し、最後に総合結果、緊急判断、翌営業日の注文候補、人間が行うこと、保存先を簡潔に報告してください。
~~~

## 5. 1回の実行フロー

1. `prepare` を実行し、同日の未完了実行があれば再開する
2. `state.json`、保有、監視、未処理注文、前回引き継ぎを読む
...
9. 翌営業日の注文候補を `orders.csv` へ `PROPOSED` で記録する
...
- `PROPOSED` が人間の承認なしに実行済み扱いになっていないか
