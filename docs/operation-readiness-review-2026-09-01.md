...
| 長期・非デイトレ | **PASS** | 日次は例外確認、候補集合は月次、企業進捗は四半期。v0.4が引き継ぐv0.2の長期ルールを維持する。 |
| 全額テンバガー運用 | **PAPERで検証開始** | 運用資産100%、安全資産0%。未投資分は新規・追加購入の戦略待機資金として扱う。 |
| ピンポイント注文不可 | **PAPERは可／現行LIVEはNO-GO** | 夜間は注文案まで。現行v0.4が引き継ぐ翌朝8:45〜8:55確認を行えないならLIVEへ昇格しない。 |
...
したがってLIVEは、コードや一括スクリプトを実行するだけではGOにならない。履歴再生と最低12か月PAPERは別々に要求し、個人条件と最終承認も自動で真にしない。v0.4の12か月は、最初の有効なv0.4前向きPAPER runから数える。
